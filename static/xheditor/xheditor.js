(function(e, $) {
	...
	var I = navigator.userAgent.toLowerCase(),
		Ba = -1 !== I.indexOf("mobile"),
		J = e.browser,
		pa = parseFloat(J.version),
		h = J.msie,
		qa = J.mozilla,
		R = J.safari,
		Ca = J.opera,
		eb = -1 < I.indexOf(" adobeair/"),
		Da = /OS 5(_\d)+ like Mac OS X/i.test(I);
	e.fn.xheditor = function(h) {
		...
		this.each(function() {
			...
			} else if (this.xheditor) o.push(this.xheditor);
			else {
	...
	if (h) {
		...
		} catch (qb) {}(I = e.fn.jquery) && I.match(/^1\.[67]/) && (e.attrHooks.width = e.attrHooks.height = null)
	}
	var fb = {
		27: "esc",
		9: "tab",
		...
		ra = function(z, o) {
			...
			function Ua(a) {
				...
				setTimeout(function() {
					...
					m.remove();
					d.loadBookmark();
					if (b = b.replace(/^[\s\uFEFF]+|[\s\uFEFF]+$/g, "")) if (a) d.pasteText(b);
					else if (b = d.cleanHTML(b), b = d.cleanWord(b), b = d.formatXHTML(b), !g.onPaste || g.onPaste && !1 !== (b = g.onPaste(b))) b = Va(b), d.pasteHTML(b);
					Ja = !1
				}, 0)
			}
			function Va(a) {
				var b = g.localUrlTest,
					c = g.remoteImgSaveUrl;
				if (b && c) {
					var f = [],
						i = 0,
						a = a.replace(/(<img)((?:\s+[^>]*?)?(?:\s+src="\s*([^"]+)\s*")(?: [^>]*)?)(\/?>)/ig, function(a, c, d, e, s) {
			...
			function Ba(a) {
				...
				if (a.is("li")) return !0;
				if (g.forcePtag) 0 === a.length && d._exec("formatblock", "<p>");
				else return d.pasteHTML("<br />"), h && 0 < a.length && 2 === d.getRng().parentElement().childNodes.length && d.pasteHTML("<br />"), !1
			}
			function Na() {
			...
			function L(a, b, c) {
				if (!b) return !1;
				var d = "_xhe_" + b;
				c && (va && (c = V(c, va, F)), a.attr(b, F ? V(c, "abs", F) : c).removeAttr(d).attr(d, c));
			...
			function M(a, b) {
				var c = typeof a;
				return !b ? "undefined" != c : "array" === b && a.hasOwnProperty && a instanceof Array ? !0 : c === b
			}
			function V(a, b, c) {
				if (a.match(/^(\w+):\/\//i) && !a.match(/^https?:/i) || /^#/i.test(a) || /^data:/i.test(a)) return a;
				var d = c ? e('<a href="' + c + '" />')[0] : location,
					c = d.protocol,
					i = d.host,
					m = d.hostname,
					j = d.port,
					d = d.pathname.replace(/\\/g, "/").replace(/[^\/]+$/i, "");
				"" === j && (j = "80");
				"" === d ? d = "/" : "/" !== d.charAt(0) && (d = "/" + d);
				a = e.trim(a);
				"abs" !== b && (a = a.replace(RegExp(c + "\\/\\/" + m.replace(/\./g, "\\.") + "(?::" + j + ")" + ("80" === j ? "?" : "") + "(/|$)", "i"), "/"));
				"rel" === b && (a = a.replace(RegExp("^" + d.replace(/([\/\.\+\[\]\(\)])/g, "\\$1"), "i"), ""));
			...
			var d = this,
				...
				X = null,
				O = 0,
				g = d.settings = e.extend({}, ra.settings, o),
				xa = g.plugins,
				ya = [];
			...
			var ab = "xheCSS_" + g.skin,
				...
				bb = "xhe" + aa + "_Tool",
				cb = "xhe" + aa + "_iframearea",
				db = "xhe" + aa + "_iframe",
				za = "xhe" + aa + "_fixffcursor",
				P = "",
				Q = "",
				ua = A + "xheditor_skin/" + g.skin + "/",
				Aa = kb,
				va = g.urlType,
				F = g.urlBase,
				Y = g.emotPath,
				Y = Y ? Y : A + "xheditor_emot/",
				Ra = "",
				Aa = e.extend({}, Aa, g.emots),
				Y = V(Y, "rel", F ? F : null);
			...
			this.init = function() {
				...
				e.each(g.tools, function(a, b) {
					...
					if ("|" === b) c.push('<span class="xheSeparator"/>');
					else if ("/" === b) c.push("<br />");
					else {
						f = ka[b];
						if (!f) return;
					...
					b.match(m) && c.push('<span class="xheGStart"/>')
				});
				...
				if (a = g.loadCSS) if (M(a, "array")) for (var j in a) P += '<link rel="stylesheet" href="' + a[j] + '"/>';
				else P = a.match(/\s*<style(\s+[^>]*?)?>[\s\S]+?<\/style>\s*/i) ? P + a : P + ('<link rel="stylesheet" href="' + a + '"/>');
				j = "<html><head>" + P + "<title>\u53ef\u89c6\u5316\u7f16\u8f91\u5668,alt+1\u52309\u952e,\u5207\u6362\u5230\u5de5\u5177\u533a,tab\u952e,\u9009\u62e9\u6309\u94ae,esc\u952e,\u8fd4\u56de\u7f16\u8f91 " + (g.readTip ? g.readTip : "") + "</title>";
				...
				n.append(c.join("")).bind("mousedown contextmenu", N).click(function(a) {
					...
					b.is(".xheEnabled") && (clearTimeout(wa), n.find("a").attr("tabindex", "-1"), v = a, d.exec(b.attr("cmd")));
					return !1
				});
				n.find(".xheButton").hover(function(a) {
					...
					if (ia) {
						var b = a.clientX - ia.x,
							c = a.clientY - ia.y;
						if (1 < Math.abs(b) || 1 < Math.abs(c)) 0 < b && 0 < c ? (b = Math.round(Math.atan(c / b) / 0.017453293), X = X ? (X + b) / 2 : b) : X = null, ia = {
							x: a.clientX,
							y: a.clientY
						}
					}
				...
				ga.keydown(function(a) {
					var b = a.which;
					if (a.altKey && 49 <= b && 57 >= b) return n.find("a").attr("tabindex", "0"), n.find(".xheGStart").eq(b - 49).next().find("a").focus(), l.title = "\ufeff\ufeff", !1
				}).click(function() {
					n.find("a").attr("tabindex", "-1")
				});
				n.keydown(function(a) {
					var b = a.which;
					if (27 == b) n.find("a").attr("tabindex", "-1"), d.focus();
					else if (a.altKey && 49 <= b && 57 >= b) return n.find(".xheGStart").eq(b - 49).next().find("a").focus(), !1
				});
				...
				g.html5Upload && j.bind("dragenter dragover", function(a) {
					var b;
					...
					if (a && (b = a.files) && 0 < b.length) {
						var c, k, a = ["Link", "Img", "Flash", "Media"],
							f = [],
							j;
						...
						k = function(a) {
							var b, d;
							for (c = 0; c < a.length; c++) if (b = a[c].name.replace(/.+\./, ""), b = j.match(RegExp("(\\w+):[^:]*," + b + "(?:,|$)", "i"))) if (d) {
								if (d !== b[1]) return 2
							} else d = b[1];
							else return 1;
							return d
						}(b);
						1 === k ? alert("\u4e0a\u4f20\u6587\u4ef6\u7684\u6269\u5c55\u540d\u5fc5\u9700\u4e3a\uff1a" + j.replace(/\w+:,/g, "")) : 2 === k ? alert("\u6bcf\u6b21\u53ea\u80fd\u62d6\u653e\u4e0a\u4f20\u540c\u4e00\u7c7b\u578b\u6587\u4ef6") : k && d.startUpload(b, g["up" + k + "Url"], "*", function(a) {
							var b = [],
								c;
							(c = g.onUpload) && c(a);
			...
			this.setTextCursor = function(a) {
				var b = d.getRng(!0),
					c = l.body;
				if (h) b.moveToElementText(c);
				else {
					for (var e = a ? "lastChild" : "firstChild"; 3 != c.nodeType && c[e];) c = c[e];
					b.selectNode(c)
				}
				b.collapse(a ? !1 : !0);
			...
			this.getParent = function(a) {
				var b = d.getRng(),
					c;
				h ? c = b.item ? b.item(0) : b.parentElement() : (c = b.commonAncestorContainer, b.collapsed || b.startContainer === b.endContainer && 2 > b.startOffset - b.endOffset && b.startContainer.hasChildNodes() && (c = b.startContainer.childNodes[b.startOffset]));
				a = a ? a : "*";
				c = e(c);
			...
			this.getSelect = function(a) {
				var b = d.getSel(),
					c = d.getRng(),
					f = !0,
					f = !c || c.item ? !1 : !b || 0 === c.boundingWidth || c.collapsed;
				if ("text" === a) return f ? "" : c.text || (b.toString ? b.toString() : "");
				c.cloneContents ? (a = e("<div></div>"), (c = c.cloneContents()) && a.append(c), c = a.html()) : c = M(c.item) ? c.item(0).outerHTML : M(c.htmlText) ? c.htmlText : c.toString();
				f && (c = "");
			...
			this.processHTML = function(a, b) {
				if ("write" === b) {
					a = a.replace(/(<(\/?)(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*((\/?)>)/g, function(a, b, c, d, e, f, i) {
						...
						var k, y = "";
						if ("del" === d) d = "strike";
						else if ("img" === d) e = e.replace(/\s+emot\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i, function(a, b) {
							k = b.match(/^(["']?)(.*)\1/)[2];
							k = k.split(",");
							k[1] || (k[1] = k[0], k[0] = "");
							"default" === k[0] && (k[0] = "");
						...
						else if ("a" === d)!e.match(/ href=[^ ]/i) && e.match(/ name=[^ ]/i) && (y += " xhe-anchor"), i && (f = "></a>");
						else if ("table" === d && !c && (a = e.match(/\s+border\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i), !a || a[1].match(/^(["']?)\s*0\s*\1$/))) y += " xhe-border";
						var Z, e = e.replace(/\s+([\w\-:]+)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/g, function(a, b, c) {
							b = b.toLowerCase();
							c = c.match(/^(["']?)(.*)\1/)[2];
							aft = "";
						...
						k && (a = Y + (k[0] ? k[0] : "default") + "/" + k[1] + ".gif", e += ' src="' + a + '" _xhe_src="' + a + '"');
					...
					if (!R) var c = function(a, b, c, d, e, f) {
							...
							if (k = d.match(/(?:^|[\s;])color\s*:\s*([^;"]+)/i)) {
								if (i = k[1].match(/\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i)) {
									k[1] = "#";
					...
					if (R) for (var d = [{
						...
						}, e = 0; 2 > e; e++) a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig, c), a = a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/ig, c);
					a = a.replace(/(<(\w+))((?:\s+[\w\-:]+\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))*)\s*(\/?>)/g, function(a, b, c, d, e) {
						var c = c.toLowerCase(),
							f, d = d.replace(/\s+_xhe_(?:src|href)\s*=\s*("[^"]*"|'[^']*'|[^>\s]+)/i, function(a, b) {
								f = b.match(/^(["']?)(.*)\1/)[2];
								return ""
							});
						f && va && (f = V(f, va, F));
			...
			this.cleanWord = function(a) {
				...
				if (0 < b && 3 > b && /mso(-|normal)|WordDocument|<table\s+[^>]*?x:str|\s+class\s*=\s*"?xl[67]\d"/i.test(a)) {
					...
					for (var a = a.replace(/(<(\/?)([\w\-:]+))((?:\s+[\w\-:]+(?:\s*=\s*(?:"[^"]*"|'[^']*'|[^>\s]+))?)*)\s*(\/?>)/g, function(a, c, d, e, f, g) {
						e = e.toLowerCase();
						if (e.match(/^(link)$/) && f.match(/file:\/\//i) || e.match(/:/) || "span" === e && 2 === b) return "";
					...
					for (c = 0; 3 > c; c++) a = a.replace(/<(span|a)>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig, d);
			...
			this.cleanHTML = function(a) {
				var a = a.replace(/<!?\/?(DOCTYPE|html|body|meta)(\s+[^>]*?)?>/ig, ""),
					b, a = a.replace(/<head(?:\s+[^>]*?)?>([\s\S]*?)<\/head>/i, function(a, d) {
			...
			this.formatXHTML = function(a, b) {
				...
				function g(a) {
					if (a) for (b = E.length - 1; 0 <= b && !(E[b] === a); b--);
					else var b = 0;
					if (0 <= b) {
				...
				function w(a, c, d) {
					B || (a = a.replace(/(\t*\r?\n\t*)+/g, ""));
					if (!B && !0 === b) if (a.match(/^\s*$/)) H.push(a);
					else {
						var e = k[c];
						e ? (d && v++, "" === A && v--) : A && v++;
						((e ? c : "") !== A || e) && s();
				...
				function r(a, b, c, d) {
					if (!c) return d;
					var e = "",
						c = c.replace(/ face\s*=\s*"\s*([^"]*)\s*"/i, function(a, b) {
							b && (e += "font-family:" + b + ";");
							return ""
						}),
				...
				var D = c("area,base,basefont,br,col,frame,hr,img,input,isindex,link,meta,param,embed"),
					k = c("address,applet,blockquote,button,center,dd,dir,div,dl,dt,fieldset,form,frameset,h1,h2,h3,h4,h5,h6,hr,iframe,ins,isindex,li,map,menu,noframes,noscript,object,ol,p,pre,table,tbody,td,tfoot,th,thead,tr,ul,script"),
					y = c("a,abbr,acronym,applet,b,basefont,bdo,big,br,button,cite,code,del,dfn,em,font,i,iframe,img,input,ins,kbd,label,map,object,q,s,samp,script,select,small,span,strike,strong,sub,sup,textarea,tt,u,var"),
					Z = c("colgroup,dd,dt,li,options,p,td,tfoot,th,thead,tr"),
				...
				for (var u, C, q = 0, t, z, v = -1, A = "body", B = !1; u = n.exec(a);) {
					C = u.index;
					C > q && (q = a.substring(q, C), t ? z.push(q) : j(q));
					q = n.lastIndex;
				...
				a = a.replace(/<(font)(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig, r);
			...
			this.toggleSource = function(a) {
				if (x !== a) {
					...
					var b = l.body,
						...
						f, i, a = 0,
						g = "";
					if (x) f = d.getSource(), c.html("").removeAttr("scroll").attr("class", "editMode" + Q), h ? b.contentEditable = "true" : l.designMode = "On", qa && (d._exec("inserthtml", "-"), e("#" + za).show().focus().hide()), g = "\u6e90\u4ee3\u7801";
					else {
			...
			this.showPreview = function() {
				var a = g.beforeSetSource,
					b = d.getSource();
				a && (b = a(b));
				var a = "<html><head>" + P + "<title>\u9884\u89c8</title>" + (F ? '<base href="' + F + '"/>' : "") + "</head><body>" + b + "</body></html>",
					b = window.screen,
			...
			this.showMenu = function(a, b) {
				...
				c.click(function(a) {
					a = a.target;
					if (!e.nodeName(a, "DIV")) return d.loadBookmark(), b(e(a).closest("a").attr("v")), d.hidePanel(), !1
				}).mousedown(N);
				d.saveBookmark();
				d.showPanel(c)
			};
			this.showColor = function(a) {
				...
				b.click(function(b) {
					b = b.target;
					if (e.nodeName(b, "A")) return d.loadBookmark(), a(e(b).attr("xhev")), d.hidePanel(), !1
				}).mousedown(N);
				d.saveBookmark();
				d.showPanel(b)
			};
				...
				b.click(function() {
					...
					if ("" !== a && "http://" !== a) {
						...
						if (1 < b.length) {
							...
							d.pasteHTML(Z.join("&nbsp;"))
						} else a = b[0].split("||"), f || (f = a[0]), f = a[1] ? a[1] : "" !== w ? "" : f ? f : a[0], 0 === i.length ? (f ? d.pasteHTML('<a href="#xhe_tmpurl">' + f + "</a>") : d._exec("createlink", "#xhe_tmpurl"), i = e('a[href$="#xhe_tmpurl"]', l)) : f && !R && i.text(f), L(i, "href", a[0]), "" !== c ? i.attr("target", c) : i.removeAttr("target")
					}
					d.hidePanel();
					return !1
				});
				...
				r.click(function() {
					...
					if ("" !== a && "http://" !== a) {
						...
						if (1 < g.length) {
							...
							var u = u + " />",
								v;
							for (v in g) a = g[v], "" !== a && (a = a.split("||"), k = u, k = k.replace("xhe_tmpurl", a[0]), a[1] && (k = '<a href="' + a[1] + '" target="_blank">' + k + "</a>"), t.push(k));
							d.pasteHTML(t.join("&nbsp;"))
						} else 1 === g.length && (a = g[0], "" !== a && (a = a.split("||"), 0 === b.length && (d.pasteHTML('<img src="' + a[0] + '#xhe_tmpurl" />'), b = e('img[src$="#xhe_tmpurl"]', l)), L(b, "src", a[0]), "" !== k && b.attr("alt", k), "" !== D ? b.attr("align", D) : b.removeAttr("align"), "" !== r ? b.attr("width", r) : b.removeAttr("width"), "" !== h ? b.attr("height", h) : b.removeAttr("height"), "" !== o ? b.attr("border", o) : b.removeAttr("border"), "" !== q ? b.attr("vspace", q) : b.removeAttr("vspace"), "" !== n ? b.attr("hspace", n) : b.removeAttr("hspace"), a[1] && (g = b.parent("a"), 0 === g.length && (b.wrap("<a></a>"), g = b.parent("a")), L(g, "href", a[1]), g.attr("target", "_blank"))))
			...
			this.showEmbed = function(a, b, c, f, g, m, j) {
				var b = e(b),
					p = d.getParent('embed[type="' + c + '"],embed[classid="' + f + '"]'),
					w = e("#xhe" + a + "Url", b),
					s = e("#xhe" + a + "Width", b),
					r = e("#xhe" + a + "Height", b),
					a = e("#xheSave", b);
				m && d.uploadInit(w, m, j);
				1 === p.length && (w.val(L(p, "src")), s.val(p.attr("width")), r.val(p.attr("height")));
				a.click(function() {
					d.loadBookmark();
					var a = w.val();
					if ("" !== a && "http://" !== a) {
						...
						m.test(b) || (b = 412);
						m.test(j) || (j = 300);
						var h = '<embed type="' + c + '" classid="' + f + '" src="xhe_tmpurl"' + g,
							m = a.split(" ");
						if (1 < m.length) {
							var o, q = [],
								h = h + ' width="xhe_width" height="xhe_height" />',
								n;
							for (n in m) a = m[n].split("||"), o = h, o = o.replace("xhe_tmpurl", a[0]), o = o.replace("xhe_width", a[1] ? a[1] : b), o = o.replace("xhe_height", a[2] ? a[2] : j), "" !== a && q.push(o);
							d.pasteHTML(q.join("&nbsp;"))
			...
			this.showEmot = function(a) {
				var b = e('<div class="xheEmot"></div>'),
					...
					s = c.count,
					c = c.list;
				if (s) for (c = 1; c <= s; c++) g++, m.push("<a href=\"javascript:void('" + c + '\')" style="background-image:url(' + f + c + '.gif);" emot="' + a + "," + c + '" xhev="" title="' + c + '" role="option">&nbsp;</a>'), 0 === g % w && m.push("<br />");
				else e.each(c, function(b, c) {
					g++;
					m.push("<a href=\"javascript:void('" + c + '\')" style="background-image:url(' + f + b + '.gif);" emot="' + a + "," + b + '" title="' + c + '" xhev="' + c + '" role="option">&nbsp;</a>');
					0 === g % w && m.push("<br />")
				});
				var s = w * (j + 12),
					c = Math.ceil(g / w) * (p + 12),
					r = 0.75 * s;
				c <= r && (r = "");
				...
				e.each(Aa, function(b, c) {
					h++;
					k.push("<li" + (a === b ? ' class="cur"' : "") + ' role="presentation"><a href="javascript:void(\'' + c.name + '\')" group="' + b + '" role="tab" tabindex="0">' + c.name + "</a></li>")
				});
			...
			this.showTable = function() {
				var a = e('<div><label for="xheTableRows">\u884c\u3000\u3000\u6570: </label><input type="text" id="xheTableRows" style="width:40px;" value="3" /> <label for="xheTableColumns">\u5217\u3000\u3000\u6570: </label><input type="text" id="xheTableColumns" style="width:40px;" value="2" /></div><div><label for="xheTableHeaders">\u6807\u9898\u5355\u5143: </label><select id="xheTableHeaders"><option selected="selected" value="">\u65e0</option><option value="row">\u7b2c\u4e00\u884c</option><option value="col">\u7b2c\u4e00\u5217</option><option value="both">\u7b2c\u4e00\u884c\u548c\u7b2c\u4e00\u5217</option></select></div><div><label for="xheTableWidth">\u5bbd\u3000\u3000\u5ea6: </label><input type="text" id="xheTableWidth" style="width:40px;" value="200" /> <label for="xheTableHeight">\u9ad8\u3000\u3000\u5ea6: </label><input type="text" id="xheTableHeight" style="width:40px;" value="" /></div><div><label for="xheTableBorder">\u8fb9\u6846\u5927\u5c0f: </label><input type="text" id="xheTableBorder" style="width:40px;" value="1" /></div><div><label for="xheTableCellSpacing">\u8868\u683c\u95f4\u8ddd: </label><input type="text" id="xheTableCellSpacing" style="width:40px;" value="1" /> <label for="xheTableCellPadding">\u8868\u683c\u586b\u5145: </label><input type="text" id="xheTableCellPadding" style="width:40px;" value="1" /></div><div><label for="xheTableAlign">\u5bf9\u9f50\u65b9\u5f0f: </label><select id="xheTableAlign"><option selected="selected" value="">\u9ed8\u8ba4</option><option value="left">\u5de6\u5bf9\u9f50</option><option value="center">\u5c45\u4e2d</option><option value="right">\u53f3\u5bf9\u9f50</option></select></div><div><label for="xheTableCaption">\u8868\u683c\u6807\u9898: </label><input type="text" id="xheTableCaption" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="\u786e\u5b9a" /></div>'),
					b = e("#xheTableRows", a),
					c = e("#xheTableColumns", a),
					f = e("#xheTableHeaders", a),
					g = e("#xheTableWidth", a),
					m = e("#xheTableHeight", a),
					j = e("#xheTableBorder", a),
					p = e("#xheTableCellSpacing", a),
					h = e("#xheTableCellPadding", a),
					s = e("#xheTableAlign", a),
					r = e("#xheTableCaption", a);
				e("#xheSave", a).click(function() {
					d.loadBookmark();
					var a = r.val(),
						e = j.val(),
						...
						z = h.val(),
						x = s.val(),
						e = "<table" + ("" !== e ? ' border="' + e + '"' : "") + ("" !== n ? ' width="' + n + '"' : "") + ("" !== t ? ' height="' + t + '"' : "") + ("" !== v ? ' cellspacing="' + v + '"' : "") + ("" !== z ? ' cellpadding="' + z + '"' : "") + ("" !== x ? ' align="' + x + '"' : "") + ">";
			...
			this.uploadInit = function(a, b, c) {
				...
				var i = e('<span class="xheUpload"><input type="text" style="visibility:hidden;" tabindex="-1" /><input type="button" value="' + g.upBtnText + '" class="xheBtn" tabindex="-1" /></span>'),
					m = e(".xheBtn", i),
					j = g.html5Upload,
					p = j ? g.upMultiple : 1;
				a.after(i);
				...
				if ("!" === b.substr(0, 1)) m.click(function() {
					d.showIframeModal("\u4e0a\u4f20\u6587\u4ef6", b.substr(1), f, null, null)
				});
				else {
			...
			this.startUpload = function(a, b, c, f) {
				function i(a, c) {
					...
					e.err === $ || e.msg === $ ? alert(b + " \u4e0a\u4f20\u63a5\u53e3\u53d1\u751f\u9519\u8bef\uff01\r\n\r\n\u8fd4\u56de\u7684\u9519\u8bef\u5185\u5bb9\u4e3a: \r\n\r\n" + a) : e.err ? alert(e.err) : (m.push(e.msg), g = !0);
					(!g || c) && d.removeModal();
					c && g && f(m);
					return g
				}
				var m = [],
					j = g.html5Upload,
					p = j ? g.upMultiple : 1,
					h, s = e('<div style="padding:22px 0;text-align:center;line-height:30px;">\u6587\u4ef6\u4e0a\u4f20\u4e2d\uff0c\u8bf7\u7a0d\u5019\u2026\u2026<br /></div>'),
					r = '<img src="' + ua + 'img/loading.gif">';
				...
				d.showModal("\u6587\u4ef6\u4e0a\u4f20\u4e2d(Esc\u53d6\u6d88\u4e0a\u4f20)", s, 320, 150);
				c.start()
			};
			this.html4Upload = function(a, b, c) {
				var d = "jUploadFrame" + (new Date).getTime(),
					g = this,
					h = e('<iframe name="' + d + '" class="xheHideArea" />').appendTo("body"),
					j = e('<form action="' + b + '" target="' + d + '" method="post" enctype="multipart/form-data" class="xheHideArea"></form>').appendTo("body"),
					p = e(a),
					l = p.clone().attr("disabled", "true");
				p.before(l).appendTo(j);
				this.remove = function() {
					null !== g && (l.before(p).remove(), h.remove(), j.remove(), g = null)
				};
				this.onLoad = function() {
					var a = h[0].contentWindow.document,
						b = e(a.body).text();
					a.write("");
					g.remove();
					c(b, !0)
			...
			this.html5Upload = function(a, b, c, d, e) {
				...
				for (var h, l = 0, s = b.length, r = 0, o = 0, k = this, n = 0; n < s; n++) o += b[n].size;
				this.remove = function() {
					h && (h.abort(), h = null)
				};
			...
			this.showIframeModal = function(a, b, c, f, g, h) {
				...
				var b = e('<iframe frameborder="0" src="' + b.replace(/{editorRoot}/ig, A) + (/\?/.test(b) ? "&" : "?") + "parenthost=" + location.host + '" style="width:100%;height:100%;display:none;" /><div class="xheModalIfmWait"></div>'),
					o = b.eq(0),
					s = b.eq(1);
				d.showModal(a, b, f, g, h);
				var r = o[0].contentWindow,
					n;
				j();
				o.load(function() {
					j();
			...
			this.showModal = function(a, b, c, f, i) {
				...
				c = c ? c : g.modalWidth;
				f = f ? f : g.modalHeight;
				K = e('<div class="xheModal" style="width:' + (c - 1) + "px;height:" + f + "px;margin-left:-" + Math.ceil(c / 2) + "px;" + (h && 7 > pa ? "" : "margin-top:-" + Math.ceil(f / 2) + "px") + '">' + (g.modalTitle ? '<div class="xheModalTitle"><span class="xheModalClose" title="\u5173\u95ed (Esc)" tabindex="0" role="button"></span>' + a + "</div>" : "") + '<div class="xheModalContent"></div></div>').appendTo("body");
				Fa = e('<div class="xheModalOverlay"></div>').appendTo("body");
				0 < ea && (Ea = e('<div class="xheModalShadow" style="width:' + K.outerWidth() + "px;height:" + K.outerHeight() + "px;margin-left:-" + (Math.ceil(c / 2) - ea - 2) + "px;" + (h && 7 > pa ? "" : "margin-top:-" + (Math.ceil(f / 2) - ea - 2) + "px") + '"></div>').appendTo("body"));
			...
			this.showDialog = function(a, b) {
				var c = e('<div class="xheDialog"></div>'),
					f = e(a),
					i = e("#xheSave", f);
				if (1 === i.length) {
					...
					i.after(' <input type="button" id="xheCancel" value="\u53d6\u6d88" />');
					e("#xheCancel", f).click(d.hidePanel);
					if (!g.clickCancelDialog) {
						sa = !1;
						var h = e('<div class="xheFixCancel"></div>').appendTo("body").mousedown(N),
							j = B.offset();
						h.css({
							left: j.left,
							top: j.top,
			...
			this._exec = function(a, b, c) {
				c || d.focus();
				return b !== $ ? l.execCommand(a, !1, b) : l.execCommand(a, !1, null)
			}
		};
	ra.settings = {
		skin: "default",
	...
	e(function() {
		...
		e("textarea").each(function() {
			var h = e(this),
				o = h.attr("class");
			if (o && (o = o.match(/(?:^|\s)xheditor(?:\-(m?full|simple|mini))?(?:\s|$)/i))) h.xheditor(o[1] ? {
				tools: o[1]
			} : null)
		})
	})
